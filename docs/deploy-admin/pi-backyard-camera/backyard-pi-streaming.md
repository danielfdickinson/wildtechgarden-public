---
slug: backyard-pi-streaming
aliases:
  - /post/pi-backyard-camera/backyard-pi-streaming/
  - /deploy-admin/pi-backyard-camera/backyard-pi-streaming/
  - /backyard-pi-sub-pages/backyard-pi-streaming/
title: "Streaming From Pi Backyard Camera"
...
Obviously you need to install motion. On Pi OS you would execute ``sudo apt install motion`` and then edit the configuration (see below).
...
# The max depends on version of Pi you have; keep it low for older
# models (is in frames per second)
framerate 30

threshold_tune on

# Dynamically mask out areas of the image during operation.
# Set to 0 if it's interfering with detecting motion you want to detect.
# Useful for eliminating repetitive motion like trees swaying in the wind.
smart_mask_speed 4

# Included so clouds passing over don't trigger motion
...
# If stream_port is non-zero, the setting below allows access
# from another device (otherwise you can only view the video
# stream from the Pi itself).
stream_localhost off

# Disables webcontrol; it's safer that way
webcontrol_port 0
```
...
The second file to edit is ``/etc/default/motion``. Change it to:

```bash
start_motion_daemon=yes
```

Then execute: ``sudo systemctl restart motion``

## Allow Access to Motion (Internet to Local Pi)

### Security

* These are the least secure options for your personal network, but may have less dramatic ways of shooting yourself in the foot than using OneDrive or Google Drive, if you use those for private data (should you make an error with OneDrive or Google Drive configuration).
* You absolutely must keep your Pi OS up to date.
* If the Pi becomes compromised (e.g. through a flaw in motion or the Linux kernel which an attacker exploits) it becomes a platform to attack your network.
  * They probably don't care about you personally, but ransomware, botnets, and stealing CPU to mine crypto-currency are examples of why any incoming connection is attractive to attackers, irrespective of who you are and how uninteresting you think you may be.

### Port Forwarding

**NB** This does not detail adding SSL certificates, which means your connection is unencrypted. This is not recommended even for a public camera, so you should lookup the use of **certbot** and also configure the certificates in ``/etc/motion/motion.conf``.

* A domain and dynamic DNS or a static IP address and static DNS
  * So others can find your site
* Opening ports in your firewall (Pi), if you enabled a firewall.
  * There is no firewall on the Pi by default, so presumably if there is one you added it and know how to add an exception.
* Forwarding port from router to Pi
  * Corollary: You need admin access to your router
  * Corollary: Your ISP must allow you to operate a server
  * See [How to Set Up Port Forwarding - Port Forward](https://portforward.com/) or a similar site.
* You will need to forward from the external port (e.g. port 80 for HTTP access) to the Pi's port 8081.

### PageKite or Alternative

* PageKite is a third-party tunnelling proxy; that means your Pi connects to the proxy, users on the internet also connect to the proxy and are tunnelled through your connection to your server (in this case 'motion').
* See the [PageKite Site](https://pagekite.net) for more information, or search for alternatives to PageKite and go there for more information.

#### Requirements

* A PageKite account (accounts are currently free for personal (non-commercial/non-organizational) use)
* PageKite software (cross-platform and easily available for the Pi)

#### Notes

* This is probably easiest option for live streaming (using motion or an always-on video stream).
* It has the advantage of not making your public IP address available.
* It is more secure than port forwarding directly into your Pi (you don't have to manage the SSL yourself), but still means remote devices are connecting to your local Pi on your local network.
