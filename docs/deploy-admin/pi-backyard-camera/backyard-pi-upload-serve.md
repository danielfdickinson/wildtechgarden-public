---
slug: backyard-pi-upload-serve
aliases:
  - /post/pi-backyard-camera/backyard-pi-upload-serve/
  - /deploy-admin/pi-backyard-camera/backyard-pi-upload-serve/
  - /backyard-pi-sub-pages/backyard-pi-upload-serve/
title: "Upload and Serve Captures From Pi Backyard Camera"
author: Daniel F. Dickinson
date: 2021-09-22T16:51:00-04:00
publishDate: 2021-09-22T16:51:00-04:00
tags:
    - hosting
    - linux
    - presentations
    - raspberry-pi
    - technology
description: Uploading and serving image and video captures from a Raspberry Pi as a backyard public camera
...
Obviously you need to install motion. For this article we also use rclone (to do the uploads). On Pi OS you would execute ``sudo apt install motion rclone`` and then edit the configuration (see below).
...
# We add the data subdir here because we don't want to copy all files from the
# motion user's home directory (/var/lib/motion), especially rclone configuration
# which includes authentication information.
target_dir /var/lib/motion/data
...
While testing it can be useful to use ``stream_port 8081`` and point your browser at ``http://<your-pi-address>:8081`` to see what the camera sees, but leaving it enabled all the time uses CPU and RAM that could reduce the quality of images and/or videos you capture and/or cause missed frames.
...
## Automatically Copy the Files to Server

Using tool called [rclone](https://rclone.org) the same method can be used for any server (including but not limited to: a local web server, a web server on a VPS, Microsoft OneDrive, or Google Drive), the only part that changes is the initial configuration for rclone to access the backend (server/service).

### Using a Script and Motion Hook to Copy Files

If you copy one of the ``copymotion`` scripts in the following sections as ``/usr/local/bin/copymotion`` and add the configuration in this section to ``/etc/motion/motion.conf`` changed files will be copied to the location specified in the script when a picture is created or a movie has been written fully to disk.

#### Configuration Additions to ``/etc/motion/motion.conf`` for Autocopy

```config
on_picture_save copymotion
on_movie_end copymotion
```

### Configuring rclone

To configure [rclone](https://rclone.org) you need to create a 'remote' for each server to which you will be copying the files. To do that for the ``motion`` user, execute ``sudo -H -u motion rclone config`` and enter the appropriate information at the prompts.

### Configuring the Server and Copying

* [Local Web Server](backyard-pi-local-web-server/)
* [Remote Web Server (e.g. VPS)](backyard-pi-remote-web-server/)
* [Microsoft OneDrive](backyard-pi-onedrive/)
* [Google Drive](backyard-pi-google-drive/)

## Serving the Files

We don't cover that here as there are plenty of guides for each type of server above.
