...
aliases:
    - /docs/arm-development/arm-libvirt-kvm-virtualization/uefi-automated-arm-for-libvirt/create-image-and-boot-files/
    - /devel/arm-devel/arm-libvirt-kvm-virtualization/uefi-automated-arm-for-libvirt/create-image-and-boot-files/
...
A Debian preseed file preseed-arm-uefi.cfg such as the one shown here, in the preseed-dir
subdirectory:
...
preseed-image-armhf-uefi-debian-10-var-file.json
...
1. Execute mkdir -p output/armhf-uefi-debian-10.6\_VARS
2. Execute truncate -s 64M output/armhf-uefi-debian-10.6\_VARS
...
2. The packer command while take a **long** time (probably over an hour and a half). To watch
...
3. Execute ``ls /boot`` to find the names of the newest vmlinuz and initrd.img (you don’t
want the plain vmlinuz and initrd.img because they are just symlinks).
...
Next: [Use the Image](use-the-image.md)
